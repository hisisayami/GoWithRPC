...
func (*Category) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case category.FieldCategoryName, category.FieldCategoryDescription:
...
func (c *Category) assignValues(columns []string, values []interface{}) error {
	...
	for i := range columns {
		...
		case category.FieldCategoryDescription:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field category_description", values[i])
			} else if value.Valid {
				c.CategoryDescription = value.String
			}
		}
	}
	return nil
...
func (c *Category) String() string {
	...
	builder.WriteString(c.CategoryDescription)
	builder.WriteByte(')')
